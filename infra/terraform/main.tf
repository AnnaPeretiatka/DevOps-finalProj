...
module "eks" {
  ...
  eks_managed_node_groups = {
    default = { instance_types=[var.node_instance_type], desired_size=var.node_desired, min_size=var.node_min, max_size=var.node_max, subnet_ids=module.vpc.private_subnets }
  }
  tags = local.tags
}
module "db" {
...
resource "aws_acm_certificate_validation" "cert" { certificate_arn = aws_acm_certificate.cert.arn validation_record_fqdns = [for r in aws_route53_record.cert_validation : r.fqdn] }
...
resource "aws_iam_role" "github_deploy" {
  ...
  assume_role_policy = jsonencode({Version="2012-10-17",Statement=[{Effect="Allow",Principal={Federated=aws_iam_openid_connect_provider.github.arn},Action="sts:AssumeRoleWithWebIdentity",Condition={StringEquals={"token.actions.githubusercontent.com:aud":"sts.amazonaws.com"},StringLike={"token.actions.githubusercontent.com:sub":"repo:*:*"}}}]})
  tags = local.tags
}
...
resource "aws_iam_role_policy_attachment" "github_deploy_attach" { role=aws_iam_role.github_deploy.name policy_arn=aws_iam_policy.deploy_policy.arn }
...
output "acm_arn"      { value = aws_acm_certificate_validation.cert.certificate_arn }
